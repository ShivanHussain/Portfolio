...
const Hero = () => {
  ...
  const socials = [
    { url: user.linkedInURL, icon: <Linkedin size={18} />, label: "LinkedIn" },
    { url: user.githubURL, icon: <Github size={18} />, label: "GitHub" },
  ].filter((s) => s.url);
    ...
    <div className="w-full grid lg:grid-cols-2 gap-16 items-center">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="fade-in-up delay-2">
          <p className="text-slate-400 text-base mb-2" style={{ fontFamily: "DM Sans" }}>
            Hi, I'm
          </p>
          <h1
          ...
          >
            {user.fullName || user.name || "Shivan Hussain"}
          </h1>
        </div>

        {/* Typewriter — DevOps titles */}
        ...
        {/* Bio */}
        <p
          className="fade-in-up delay-4 text-slate-400 text-base leading-relaxed max-w-md"
        ...
        <div className="fade-in-up delay-6 flex items-center gap-3 mt-1">
          ...
          {socials.map((s, i) => (
            <Link to={s.url} key={i} target="_blank">
              <div className="social-icon" title={s.label}>{s.icon}</div>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="hidden lg:flex justify-center items-center">
        <div className="relative float">
          ...
          {/* Main glass card */}
          <div
            className="glass rounded-3xl p-7 w-80"
